//Otra idea de mapa, posiblemente innecesaria
var mymap = L.map('map').setView([-33.448890, -70.669265], 13);

fetch('mapa.py').then( response => {
    ...
    data.Result.filter(element => element.coordinates != undefined).forEach(element => {
        ...
        marker.bindPopup(element.details.reduce((acc, val) => {
            ...
            acc = acc.concat(`<div>
                    ...
                    <br>
                    <a href="cgi-bin/info_actividad.py?id=${val.report_id}" target="_blank">(Link de la Actividad)</a>
