...
SELECT DISTINCTROW AC.id, AC.sector, AC.nombre, C.nombre, AC.email, AC.celular, AC.dia_hora_inicio, AC.dia_hora_termino, AC.descripcion, T.nombre
            FROM actividad AS AC, comuna AS C, tema AS T
            WHERE AC.comuna_id = C.id AND T.id = AC.id;